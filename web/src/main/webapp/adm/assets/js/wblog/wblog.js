...
//	var link = _form.action;
    var link = ctx + '/admin/article/postedit/'; 
    link += "?edit=1";
    if (isPub) {
        link += "&isPub=1";
    }
            ...
            if (!tools.success(ret.code)) {
                ...
                if (!isPub) {//草稿保存
                	...
                	if (!auto) {//非
                        ...
                        //location
                        location.href =  + "/article/" + ret.data + ".html";
//                        window.location.href =  + "/admin/list_blog.html";
                    }
                }
            }
        },
...
function getPostData() {
    ...
    var articleTitle = tools.val2("articleTitle");
...
//    data += "&userinfo1=" + $("#userinfo2").val();   
    ...
    var jsondata = {};
    jsondata.selTypeCode = selTypeCode;
    jsondata.articleBlogId = articleBlogId;
    jsondata.articleTitle = articleTitle;
    jsondata.articleContext = articleContext;
    jsondata.articleContext_markdown = articleContext_markdown;
    jsondata.articleTag = articleTag;
    jsondata.owenerTag = owenerTag;
    jsondata.articleDigest = articleDigest;
    if (jsondata.articleBlogId == 0) {
    	jsondata.isdraft = 1;
    } else {
    	jsondata.isdraft = $("#isdraft").val();
    }
    var data = $.toJSON(jsondata);
