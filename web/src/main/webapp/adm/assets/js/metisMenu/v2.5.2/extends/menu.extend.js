    ...
    var MetisMenu = function () {
      ...
      _createClass(MetisMenu, [{
        ...
        value: function init() {
          ...
          // 节点监控
          $(this._element).find("a").parent("li").children("a").on(Event.CLICK_DATA_API, function (e) {
        	  var _this = $(this);
        	  var _parent = _this.parent('li');
              var _list = _parent.children('a');
              
              if (self._config.preventDefault) {
                  e.preventDefault();
              }
          	
              // 非叶子节点(菜单)过滤,要求非叶子节点拥有ul, 叶子节点li不能拥有ul
              if(_parent.has("ul").length != 0){
            	  return;
              }
              // 叶子节点过滤
              if(_parent.hasClass('disabled')){
