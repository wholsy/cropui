...
<script>
    function del(id) {
        if (confirm("确定删除吗?")) {
            $.get("/adm/books_mgr/del/?_id=" + id + "", function (rs) {
                if (rs == "ok") {
    ...
    function page(pageno) {
        var txt_q = $("#txt_q").val();
        $.post("/adm/books_mgr/doshowlist/?isdraft=0&pageno=" + pageno + "&t=" + new Date().getTime() + "", {"txt_q": txt_q}, function (data) {
            console.log(data);
